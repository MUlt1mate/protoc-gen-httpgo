...
func (g *generator) genBuildRequestMethod(serviceName string, method methodParams) (err error) {
	...
	if method.withFiles {
		if err = g.getMultipartRequestServer(method); err != nil {
			return err
		}
...
func (g *generator) getMultipartRequestServer(method methodParams) (err error) {
	if *g.cfg.Library == libraryFastHTTP {
		...
		g.gf.P("r.ParseMultipartForm(32 << 20)")
	}
	for _, f := range method.inputFieldList {
		...
		case libraryNetHTTP:
			if methodField.isFile {
				...
				g.gf.P("if values := r.Form[\"", methodField.protoName, "\"]; len(values) > 0 {")
				if methodField.cardinality == protoreflect.Repeated {
					g.gf.P("	for _, value := range values {")
					if err = g.genFieldConvertor(methodField, "value", true, "nil, ", false); err != nil {
						return err
					}
					g.gf.P("	}")
				} else {
					if err = g.genFieldConvertor(methodField, "values[0]", false, "nil, ", false); err != nil {
						return err
					}
				}
				g.gf.P("}")
		...
		case libraryFastHTTP:
			if methodField.isFile {
				...
				g.gf.P("if values, ok := body.Value[\"", methodField.protoName, "\"]; ok && len(values) > 0 {")
				if methodField.cardinality == protoreflect.Repeated {
					g.gf.P("	for _, value := range values {")
					if err = g.genFieldConvertor(methodField, "value", true, "nil, ", false); err != nil {
						return err
